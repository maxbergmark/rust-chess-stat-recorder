...
impl ParallelParser {
    ...
    fn spawn_parser_thread<'a>(&'a self, scope: &Scope<'a>, game_send: Sender<Game>) {
        ...
        scope.spawn(move |_| {
            ...
            let res = core_affinity::set_for_current(CoreId{id: self.channel_id});
            if !res {
    ...
    fn get_file(&self) -> Box<dyn io::Read + Send> {
        let uncompressed: Box<dyn io::Read + Send> = if self.filename.lock().unwrap().ends_with(".zst") {
...
*/
    pub(crate) fn create_channel(&self, filename_recv: Receiver<String>) {
            for filename in filename_recv {
                let start_time = Instant::now();
                crossbeam::scope(|scope| {
                    println!("Starting consuming on thread {}: {}", self.channel_id, &filename);
                    *self.filename.lock().unwrap() = filename;
